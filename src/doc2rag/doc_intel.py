...
from .db_utils.models import SplitFile, DIRequest
from .db_utils.database import BaseSQLAgent
from .logger_utils import LoggingAgent
from .config_utils import PathConfig, DocumentIntelligenceConfig
...
class Submitter:
    ...
    async def submit_jobs(
        ...
        for split_file in split_files:
            await self._submit_single_job(session, split_file, pollers)
    ...
    async def _submit_single_job(
        ...
        try:
            ...
            if not file_path:
                return
            iobytes = self._convert_pdf_to_iobytes(file_path)
            ...
            di_request = await self._register_di_request(session, split_file.id)
            pollers.append(
                PollerDict(
    ...
    async def _register_di_request(self, session: Session, split_file_id: int) -> DIRequest:
        di_request = DIRequest(
...
class Listener:
    ...
    async def listen_jobs(self, session: Session, pollers: list):
        """
        Polls for job status and processes results until all jobs are completed.
        """
        while pollers:
            for poller_dict in pollers[:]:
                await self._process_polling(session, poller_dict, pollers)
            if pollers:
                self.logger.info(
    ...
    async def _process_polling(
        ...
        if poller.done():
            await self._handle_completed_poll(session, poller_dict)
    ...
    async def _handle_completed_poll(self, session: Session, poller_dict: PollerDict):
        poller = poller_dict.poller
...
class DIProcessor:
    ...
    async def run(self):
        with self.SessionLocal() as session:
            while not self.end_flag:
                split_files = await self._fetch_split_files_to_analyze(session)
                if not split_files:
                    self.logger.info("No files to analyze.")
                    self.end_flag = True
                    break
                await self.submitter.submit_jobs(session, split_files, self.pollers)
                await self.listener.listen_jobs(session, self.pollers)
                self.logger.info(
    ...
    async def _fetch_split_files_to_analyze(self, session: Session) -> list[SplitFile]:
        return (
