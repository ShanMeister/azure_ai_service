...
class TextTableImageBundler:
    ...
    async def bundle(self):
        """
        Process pages with the status `raw-md-extracted` and bundle tables and figures sequentially.
        """
        file_process = FileProcessClass()
        with self.SessionLocal() as session:
            ...
            merged_bundle = file_process.merge_bundle(file_dir_name)
    ...
    def _fetch_pages_to_process(
        ...
        """
        Fetch pages with the status `raw-md-extracted` and related file_dir_name.
        """
        result = (
            ...
            .filter(Page.status == "raw-md-extracted")
            .options(
...
class BundleStatusUpdateAgent:
    ...
    def _fetch_split_files_to_check(self, session: Session) -> list[SplitFile]:
        """
        Fetch split files with the status `page-split-success` and eagerly load their pages.
        """
        return (
            session.query(SplitFile)
            .options(joinedload(SplitFile.pages))
            .filter(SplitFile.status == "page-split-success")
            .all()
        )
